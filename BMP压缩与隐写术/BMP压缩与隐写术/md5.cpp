...
unsigned char* MD5::encode(unsigned char* encrypt)
{
	int j;
	MD5_CTX md5;
	MD5Init(&md5);
...
bool MD5::cmpcode(unsigned char *code1, unsigned char *code2)
{
	int i;
	for (i = 0; i < 16; i++)
	{
...
int main()
{
	...
	for (j = 0; j<16; j++)
	{
		printf("%02x", decrypt1[j]);
	}
	int i;
	for (i = 0; i < 16; i++)
	{
		if (decrypt[i] == decrypt1[i])
		{
			if (i < 15)
				continue;
		}
...
int main()
{
	...
	for (i = 0; i < 32; i++)
	{
		if (decrypt[i] == decrypt1[i])//decrypt是jiami里的变量可以在这里直接用吗
			continue;
		else
			printf("不是原始密码，无法打开图片哈哈哈哈哈哈\n");
		break;
